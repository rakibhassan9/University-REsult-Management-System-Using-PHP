<?php
	...
	if(isset($first_name) && isset($last_name) && isset($email)){
		...
		$update_data = $update_obj->updateMyProfileExamController($id, $first_name, $last_name, $email);
		...
		if($update_data){
			...
			header("LOCATION: myProfileExamController.php?status=1");
		}
		else{
			header("LOCATION: myProfileExamController.php?status=0");
		}
	}
