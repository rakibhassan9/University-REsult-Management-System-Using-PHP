<?php
	...
	if(isset($first_name) && isset($last_name) && isset($email)){
		...
		$update_data = $update_obj->updateMyProfileSuperAdmin($id, $first_name, $last_name, $email);
		...
		if($update_data){
			...
			header("LOCATION: myProfileSuperAdmin.php?status=1");
		}
		else{
			header("LOCATION: myProfileSuperAdmin.php?status=0");
		}
	}
